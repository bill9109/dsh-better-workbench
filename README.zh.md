...
点击 **创建工作台** 打开创建对话框，选择模板或应用，确认名称和配置后再创建并打开。选择或取消草稿不会新增实例。内置 **从网页地址创建** 模板支持 HTTP(S) 网址，并将网站 favicon 映射为 DSH 图标配色。单实例应用提供“打开已有”。网页隔离与兼容性边界见 [Website Workbench](docs/website-workbench.md)。
