...
import { IconCloseFill14, IconEditOutline16, IconEllipsisOutline16, IconPersonalizationOutline16, IconProjectAddOutline16, IconSearchOutline16, Menu, Tooltip, } from '@deepseek-ai/dsh-client-ui-primitives';
...
function WorkbenchRow({ instance, compact, active, draggable, dropPosition, onOpen, onRename, onDragStart, onDragOver, onDrop, onDragEnd, }) {
    ...
    const commitRename = () => {
        ...
        if (next !== '' && next !== instance.title)
            onRename(next);
        setRenaming(false);
    };
    return (_jsxs("div", { className: "dsh-workbench-sidebar-row", "data-active": active, "data-menu-open": menuOpen, "data-drop-position": dropPosition ?? undefined, role: "button", tabIndex: compact ? -1 : 0, draggable: draggable && !renaming, onClick: onOpen, onKeyDown: event => {
            if (event.key === 'Enter' || event.key === ' ') {
                event.preventDefault();
                onOpen();
            }
        }, onDragStart: onDragStart, onDragOver: onDragOver, onDrop: onDrop, onDragEnd: onDragEnd, children: [compact && _jsx(IconPersonalizationOutline16, { className: "dsh-workbench-sidebar-rail-icon" }), renaming ? (_jsx("input", { ref: input, className: "dsh-workbench-sidebar-rename", value: renameDraft, "aria-label": "\u5DE5\u4F5C\u53F0\u540D\u79F0", onChange: event => { setRenameDraft(event.target.value); }, onClick: event => { event.stopPropagation(); }, onBlur: commitRename, onKeyDown: event => {
                    if (event.key === 'Enter')
                        commitRename();
                    if (event.key === 'Escape')
                        setRenaming(false);
                } })) : (_jsx("span", { className: "dsh-workbench-sidebar-row-label", children: instance.title })), !compact && !renaming && (_jsx("span", { className: "dsh-workbench-sidebar-row-actions", children: _jsx(Menu, { open: menuOpen, onClose: () => { setMenuOpen(false); }, items: [{ id: 'rename', label: '重命名', icon: _jsx(IconEditOutline16, {}) }], onSelect: id => {
                        setMenuOpen(false);
                        if (id === 'rename') {
...
export function WorkbenchSidebar({ service }) {
    ...
    if (compact) {
        return (_jsx("section", { ref: section, className: "dsh-workbench-sidebar-section", "aria-label": "\u5DE5\u4F5C\u53F0", "data-compact": "true" }));
    }
    return (_jsxs("section", { ref: section, className: "dsh-workbench-sidebar-section", "aria-label": "\u5DE5\u4F5C\u53F0", children: [_jsxs("div", { className: "dsh-workbench-sidebar-heading", children: [_jsx("span", { className: `dsh-workbench-sidebar-heading-label${searchExpanded && !compact ? ' is-hidden' : ''}`, children: "\u5DE5\u4F5C\u53F0" }), _jsx("div", { ref: searchRoot, className: `dsh-workbench-sidebar-search-slot${searchExpanded ? ' is-expanded' : ''}`, children: _jsxs("div", { className: "dsh-workbench-sidebar-search", children: [_jsx(Tooltip, { label: "\u641C\u7D22\u5DE5\u4F5C\u53F0", side: "bottom", delayMs: 500, disabled: searchExpanded, children: _jsx("button", { type: "button", className: "dsh-workbench-sidebar-search-button", "aria-label": "\u641C\u7D22\u5DE5\u4F5C\u53F0", "aria-expanded": searchExpanded, onClick: () => {
                            ...
                            setSearchExpanded(true);
                            requestSidebarExpand();
                        }, children: _jsx(IconSearchOutline16, { size: 18 }) }) }) })), _jsxs("div", { className: "dsh-workbench-sidebar-list", "aria-label": "\u5DE5\u4F5C\u53F0\u5217\u8868", children: [_jsx(WorkbenchHomeRow, { active: snapshot.route.kind === 'workbench-home', onOpen: () => { service.openHome(); } }), instances.map(instance => (_jsx(WorkbenchRow, { instance: instance, compact: compact, active: snapshot.route.kind === 'workbench-instance' && snapshot.route.instanceId === instance.instanceId, draggable: orderBy === 'manual', dropPosition: dropTarget?.id === instance.instanceId ? dropTarget.position : null, onOpen: () => { service.open(instance.instanceId); }, onRename: title => { service.renameInstance(instance.instanceId, title); }, onDragStart: event => {
                        ...
                        }, onDrop: event => { handleDrop(instance.instanceId, event); }, onDragEnd: () => { setDraggedId(null); setDropTarget(null); } }, instance.instanceId)))] }), instances.length === 0 && (_jsx("div", { className: "dsh-workbench-sidebar-empty", children: normalizedQuery === '' ? '从首页创建实例。' : '无匹配工作台' }))] }));
}
