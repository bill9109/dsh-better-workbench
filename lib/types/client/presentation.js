function fallbackPresentation(kind) {
    if (kind === 'panel')
        return { kind: 'panel', placement: 'right', behavior: 'overlay', conversation: 'resident' };
    if (kind === 'capsule')
        return { kind: 'capsule', placement: 'floating', conversation: 'resident' };
    return { kind: 'page', conversation: 'exclusive' };
}
/** Resolve a durable route against the currently available runtime definition. */
export function resolveActivePresentation(snapshot, service) {
    if (snapshot.route.kind === 'conversation')
        return undefined;
    if (snapshot.route.kind === 'workbench-home')
        return fallbackPresentation('page');
    ...
    if (app === undefined)
        return fallbackPresentation(route.presentation);
