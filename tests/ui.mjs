...
assert.equal(pendingSaveStatus.props.className, 'dsh-workbench-save-status', 'background saves must use an out-of-flow live region')
assert.equal(pendingSaveStatus.props['aria-live'], 'polite')
assert.equal(findAll(frame.render(), item => item.props.className === 'dsh-workbench-frame-status').length, 0, 'navigation saves must not insert a status band')
const frameStyles = await readFile(new URL('../src/client/styles.ts', import.meta.url), 'utf8')
assert.ok(frameStyles.includes('.dsh-workbench-save-status { position: absolute;'))
...
const dom = new JSDOM('<!doctype html><div id="root" data-dsh-workbench-center></div>', { url: 'http://localhost/' })
...
try {
  ...
  assert.equal(rootElement.querySelector('.dsh-workbench-frame-toolbar'), null, 'renderer failures must not reintroduce page chrome')
